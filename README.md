...
- A password rotated on the server updates `.env` on the next `pull`, and `diff` reports it by name only. The previous `.env` is kept in `.secrets/` (git-ignored, newest 5 only) whenever a value in it changes.
...
- list a false positive in `channelvault.allow.json` (committed): `{ "ignore": [{ "location": "<as printed>", "kind": "assignment", "note": "why" }] }`.
